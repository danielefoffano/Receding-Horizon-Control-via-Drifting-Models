...
from drifting_mpc.utils.plotting import (
    save_cost_histogram,
    save_cost_scatter,
    save_multi_cost_histograms,
    save_multi_cost_scatter,
...
def _maybe_synchronize(device: torch.device | str | None) -> None:
    if device is None:
        return
...
def _rollout_learned(
    ...
    _maybe_synchronize(getattr(planner, "device", None))
    ...
    return np.stack(states), np.stack(actions), float(elapsed)
...
def _save_episode_table(rows: list[dict[str, object]], path: str | Path) -> Path:
...
def _build_timing_summary(prefix: str, values: np.ndarray) -> dict[str, float]:
    return {
        f"{prefix}_mean_sec": float(values.mean()),
        f"{prefix}_std_sec": float(values.std()),
        f"{prefix}_median_sec": float(np.median(values)),
    }
...
def evaluate_vs_oracle(
    ...
    regrets: list[float] = []
    learned_rollout_times: list[float] = []
    oracle_rollout_times: list[float] = []
    ...
    for episode_id, index in enumerate(episode_indices.tolist()):
        ...
        learned_states, learned_actions, learned_rollout_time = _rollout_learned(learned_env, planner, x0, omega)
        oracle_states, oracle_actions, oracle_rollout_time = _rollout_oracle(oracle_env, oracle, x0, omega)
        ...
        learned_rollout_times.append(learned_rollout_time)
        oracle_rollout_times.append(oracle_rollout_time)
        episode_rows.append(
            {
                ...
                "learned_rollout_time_sec": float(learned_rollout_time),
                "oracle_rollout_time_sec": float(oracle_rollout_time),
            }
        )
    ...
    learned_rollout_time_array = np.asarray(learned_rollout_times, dtype=np.float64)
    oracle_rollout_time_array = np.asarray(oracle_rollout_times, dtype=np.float64)
    ...
    metrics = {
        ...
        **_build_timing_summary("learned_rollout_time", learned_rollout_time_array),
        **_build_timing_summary("oracle_rollout_time", oracle_rollout_time_array),
    }
    ...
    histogram_path = save_cost_histogram(learned_array, oracle_array, eval_dir / "cost_histogram.pdf")
    scatter_path = save_cost_scatter(learned_array, oracle_array, eval_dir / "cost_scatter.pdf")
    ...
    for episode_id, learned_states, learned_actions, oracle_states, oracle_actions in rollout_bundle[: int(config["evaluation"]["num_rollout_plots"])]:
        rollout_paths.append(
            save_rollout_plot(
                ...
                path=eval_dir / f"rollout_{episode_id:03d}.pdf",
            )
        )
...
def _build_metric_summary(
    label: str,
    learned_array: np.ndarray,
    oracle_array: np.ndarray,
    regret_array: np.ndarray,
    variant: str,
    learned_rollout_time_array: np.ndarray,
    oracle_rollout_time_array: np.ndarray,
...
def evaluate_multiple_vs_oracle(
    ...
    for ckpt_path in ckpt_paths:
        planner, ckpt_config = load_planner_from_checkpoint(
            ckpt_path,
            device=device,
            m_plan=m_plan if m_plan is not None else (int(config["evaluation"]["m_plan"]) if config is not None else None),
        )
    ...
    method_rollout_times: dict[str, list[float]] = {label: [] for label in labels}
    episode_rows: list[dict[str, object]] = []
    ...
    for episode_id, index in enumerate(episode_indices.tolist()):
        ...
        for env, label, (_, planner, _ckpt_config, variant) in zip(learned_envs, labels, planners):
            learned_states, learned_actions, learned_rollout_time = _rollout_learned(env, planner, x0, omega)
            learned_cost = trajectory_cost_np(learned_states, learned_actions, omega)
            regret = (learned_cost - oracle_cost) / max(abs(oracle_cost), 1e-8)

            method_costs[label].append(learned_cost)
            method_regrets[label].append(regret)
            method_rollout_times[label].append(learned_rollout_time)
            slug = _slugify_label(label)
            row[f"cost_{slug}"] = float(learned_cost)
            row[f"regret_{slug}"] = float(regret)
            row[f"rollout_time_{slug}_sec"] = float(learned_rollout_time)
    ...
    for label, (_, _, _ckpt_config, variant) in zip(labels, planners):
        ...
        learned_rollout_time_array = np.asarray(method_rollout_times[label], dtype=np.float64)
        metrics = _build_metric_summary(
            label,
            learned_array,
            oracle_array,
            regret_array,
            variant,
            learned_rollout_time_array,
            oracle_rollout_time_array,
        )
    ...
    metrics_payload = {
        ...
        "oracle": {
            ...
            **_build_timing_summary("rollout_time", oracle_rollout_time_array),
        },
    ...
    episode_table_path = _save_episode_table(episode_rows, eval_dir / "episode_metrics.csv")
    histogram_path = save_multi_cost_histograms(
        {label: np.asarray(values, dtype=np.float32) for label, values in method_costs.items()},
        oracle_array,
        eval_dir / "cost_histograms.pdf",
    )
    scatter_path = save_multi_cost_scatter(
        {label: np.asarray(values, dtype=np.float32) for label, values in method_costs.items()},
        oracle_array,
        eval_dir / "cost_scatter.pdf",
    )
